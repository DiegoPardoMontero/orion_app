...
Una excepción: las **13 pantallas completas del MVP (Etapa 3) todavía no están dibujadas en v2**. Lo entregado aquí es:

1. **Fundamentos completos** (tokens + biblioteca de componentes con todos sus estados) — hifi, listo para construir.
2. **Dos pantallas de referencia end-to-end** en 390 y 1280 px: **Login** y **Reserva de clase** — hifi.
3. **Guía responsive por patrón** que describe, pantalla por pantalla, cómo se comporta cada layout entre 390 y 1280 px.

Para las pantallas aún no dibujadas (registro, lista de profesores, mis clases, disponibilidad del profesor, panel admin, etc.), la combinación de fundamentos + guía responsive + las dos pantallas de referencia es suficiente para implementarlas de forma consistente. Si necesitas los mocks explícitos antes de codificar, pídelos: es la Etapa 3 del proceso de diseño.
...
### Entregadas como referencia hifi
...
- `Orión Fundamentos v2.dc.html` — **el archivo principal de referencia.** Tokens, tipografía, espaciado, motion, iconografía y la biblioteca completa de componentes con todos sus estados, más la navegación por rol y la tabla responsive.
- `Orión Direcciones v2.dc.html` — las tres direcciones exploradas con login y reserva en 390 y 1280. **La dirección aprobada es `1b · Amanecer cálido premium`**; 1a y 1c quedan solo como registro de la decisión — no implementarlas.
- `support.js` — runtime del entorno de diseño, necesario para abrir los `.dc.html` en el navegador. **No es parte del producto.**

Ábrelos en un navegador y navega el archivo de Fundamentos de arriba a abajo: cada componente está dibujado en todos sus estados a tamaño real.
