...
/* ---- La mano ----
 *
 * **Una sola forma, en todas partes.** Antes había tres: el guante con costuras, un puño cerrado
 * para los gestos de un dedo, y —la peor— un dibujo a mano dentro de `BrazosDetras`, con su propio
 * arco inferior y tres líneas largas, que es la mano que saludaba en la portada. Tres manos
 * distintas en la misma mascota se notan aunque nadie sepa decir por qué.
 *
 * Ahora todas son esta, que es la que Pardo señaló como buena: la de reposo de la portada.
 *
 * Lo que se quitó y por qué:
 *
 * - **El pulgar suelto.** Era un círculo fuera de la palma y, junto a las líneas del dorso, el
 *   conjunto se leía como cuatro dedos separados. Ahora se solapa: pertenece a la mano.
 * - **El arco de abajo.** Era la costura del puño, pero cruzaba por fuera del círculo y parecía
 *   una línea perdida.
 * - **Las tres costuras largas.** Ahora son dos, cortas y dentro del dorso: se leen como costura
 *   de guante, que es lo que son, y no como dedos.
 * - **Los dedos extendidos** de señalar y de pulgar arriba. Un dedo solo saliendo de una mano de
 *   dibujo es exactamente la silueta que hay que evitar, y ninguna pose la necesita: el brazo ya
 *   dice lo que hace.
 */
  ...
  if (pose === "guia") {
    return (
      <>
        ...
        {/* El brazo extendido ya dice que señala. Un dedo saliendo de la mano no hace falta, y
            es justo la silueta que había que quitar. */}
  ...
  if (pose === "animo") {
    return (
      <>
        ...
        {/* El brazo en alto es el ánimo. El pulgar extendido se fue: por muy bien dibujado que
            estuviera, un solo dedo saliendo de una mano de dibujo es una apuesta que no hay que
            hacer, y esta la perdimos una vez en el 404. */}
...
function BrazosDetras({ pose }: { pose: RigelPose }) {
  ...
  return (
    <g className="rigel-wave">
      ...
      {/* Esta mano se dibujaba aparte, con su propio arco y tres líneas largas: era la tercera
          forma de mano del mismo personaje, y la que más se ve —saluda en la portada—. Ahora es
          la misma que todas. */}
      <Guante cx={172} cy={66} r={13} tx={181} ty={77} tr={5.5} />
    </g>
  );
}
